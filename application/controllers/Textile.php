...
class Textile extends CI_Controller {
    ...
    function get_textile_data() {
        try {
            ...
            $this->db->trans_start();
            $success_array['textile_data'] = $this->textile_model->get_all_textile_list($start, $length, $search_district, $search_entity_establishment_type, $search_logged_user_detail, $search_applicant_detail, $search_app_timing, $search_status, $search_query_status);
            $success_array['recordsTotal'] = $this->textile_model->get_total_count_of_records($search_district);
            if (($search_district != '' && (is_admin() || is_view_all_district_user())) || $search_entity_establishment_type || $search_logged_user_detail != '' || $search_applicant_detail != '' || $search_app_timing != '' || $search_status != '' || $search_query_status != '') {
                $success_array['recordsFiltered'] = $this->textile_model->get_filter_count_of_records($search_district, $search_entity_establishment_type, $search_logged_user_detail, $search_applicant_detail, $search_app_timing, $search_status, $search_query_status);
            } else {
                $success_array['recordsFiltered'] = $success_array['recordsTotal'];
            }
            $success_array['query_movements'] = $this->utility_lib->get_query_movement_string($success_array['textile_data'], VALUE_TEN);
    ...
    function submit_textile() {
        try {
            ...
            if (!is_post() || $user_id == NULL || !$user_id || $textile_id == NULL || !$textile_id ||
                    ($module_type != VALUE_ONE && $module_type != VALUE_TWO)) {
                echo json_encode(get_error_array(INVALID_ACCESS_MESSAGE));
                return false;
            }
    ...
    function _check_validation_for_textile($textile_data) {
        ...
        if (!$textile_data['entity_establishment_type']) {
    ...
    function remove_challan() {
        try {
            ...
            $file_path = 'documents' . DIRECTORY_SEPARATOR . 'textile' . DIRECTORY_SEPARATOR . $ex_est_data['challan'];
            if (file_exists($file_path)) {
                unlink($file_path);
            }
            $this->utility_model->update_data('textile_id', $textile_id, 'textile', array('status' => VALUE_TWO, 'challan' => '', 'updated_by' => $session_user_id, 'updated_time' => date('Y-m-d H:i:s')));
            $success_array = get_success_array();
    ...
    function upload_challan() {
        try {
            ...
            $payment_type = get_from_post('payment_type_for_textile_upload_challan');
            ...
            $textile_data = array();
            if ($_FILES['challan_for_textile_upload_challan']['name'] != '') {
                ...
                $temp_filename = str_replace('_', '', $_FILES['challan_for_textile_upload_challan']['name']);
                ...
                if (!move_uploaded_file($_FILES['challan_for_textile_upload_challan']['tmp_name'], $final_path)) {
                    echo json_encode(get_error_array(DOCUMENT_NOT_UPLOAD_MESSAGE));
                    return;
                }
                $textile_data['challan'] = $filename;
                $textile_data['challan_updated_date'] = date('Y-m-d H:i:s');
            }
            $textile_data['status'] = VALUE_THREE;
            if ($payment_type == VALUE_THREE) {
                $textile_data['status'] = VALUE_NINE;
            }
            $textile_data['payment_type'] = $payment_type;
            $textile_data['updated_by'] = $user_id;
            $textile_data['updated_time'] = date('Y-m-d H:i:s');
            $textile_data['total_fees'] = VALUE_ZERO;
            $this->db->trans_start();

            if ($payment_type == VALUE_ONE || $payment_type == VALUE_TWO) {
                $error_message = $this->utility_lib->update_fees_bifurcation_details(VALUE_TEN, $textile_id, $user_id, $textile_data);
                if ($error_message != '') {
                ...
                $this->utility_model->update_data('module_type', VALUE_TEN, 'fees_bifurcation', $update_data, 'module_id', $textile_id);
            }
            ...
            $success_array['total_fees'] = $textile_data['total_fees'];
            echo json_encode($success_array);
        } catch (\Exception $e) {
            echo json_encode(get_error_array($e->getMessage()));
            return false;
    ...
    function approve_application() {
        try {
            ...
            $session_user_id = get_from_session('temp_id_for_eodbsws_admin');
            $textile_id = get_from_post('textile_id_for_textile_approve');
            if (!is_post() || $session_user_id == NULL || !$session_user_id || !$textile_id || $textile_id == NULL) {
                echo json_encode(get_error_array(INVALID_ACCESS_MESSAGE));
                return false;
            }
            $update_data = array();
            $update_data['registration_number'] = get_from_post('registration_number_for_textile_approve');
            $update_data['valid_upto'] = get_from_post('valid_upto_for_textile_approve');
            $update_data['remarks'] = get_from_post('remarks_for_textile_approve');
            ...
            $this->db->trans_start();
            $ex_data = $this->utility_model->get_by_id('textile_id', $textile_id, 'textile');
            if (empty($ex_data)) {
                echo json_encode(get_error_array(INVALID_ACCESS_MESSAGE));
                return false;
            }
            if ($_FILES['certificate_file_for_textile_approve']['name'] == '') {
                echo json_encode(get_error_array(UPLOAD_DOC_MESSAGE));
                return;
            }
            $evidence_size = $_FILES['certificate_file_for_textile_approve']['size'];
            if ($evidence_size == 0) {
            ...
            $this->load->library('upload');
            $temp_filename = str_replace('_', '', $_FILES['certificate_file_for_textile_approve']['name']);
            $filename = 'textilec_' . (rand(100000000, 999999999)) . time() . '.' . pathinfo($temp_filename, PATHINFO_EXTENSION);
            //Change file name
            $final_path = $ad_path . DIRECTORY_SEPARATOR . $filename;
            if (!move_uploaded_file($_FILES['certificate_file_for_textile_approve']['tmp_name'], $final_path)) {
                echo json_encode(get_error_array(DOCUMENT_NOT_UPLOAD_MESSAGE));
                return;
            }
            $update_data['certificate_file'] = $filename;
            $temp_fc_filename = 'temp_textilefc_' . (rand(100000000, 999999999)) . time() . '.pdf';
            error_reporting(E_ERROR);
            $data = array('textile_data' => $ex_data);
            $mpdf = new \Mpdf\Mpdf(['mode' => 'utf-8', 'format' => 'A4']);
            $mpdf->WriteHTML($this->load->view('textile/qr_barcode', $data, TRUE));
            ...
            $final_certificate_filename = 'textilefc_' . (rand(100000000, 999999999)) . time() . '.pdf';
            ...
            $update_data['processing_days'] = $this->utility_lib->calculate_processing_days(VALUE_TEN, $ex_data['submitted_datetime']);
            $update_data['status'] = VALUE_FIVE;
            $update_data['status_datetime'] = date('Y-m-d H:i:s');
            $update_data['updated_by'] = $session_user_id;
            $update_data['updated_time'] = date('Y-m-d H:i:s');
            $this->utility_model->update_data('textile_id', $textile_id, 'textile', $update_data);

            $this->utility_lib->send_sms_and_email_for_app_approve($ex_data['user_id'], VALUE_SEVEN, VALUE_TEN, $textile_id);
    ...
    function reject_application() {
        try {
            ...
            $session_user_id = get_from_session('temp_id_for_eodbsws_admin');
            $textile_id = get_from_post('textile_id_for_textile_reject');
            if (!is_post() || $session_user_id == NULL || !$session_user_id || !$textile_id || $textile_id == NULL) {
                echo json_encode(get_error_array(INVALID_ACCESS_MESSAGE));
                return false;
            }
            $update_data = array();
            $update_data['remarks'] = get_from_post('remarks_for_textile_reject');
            if (!$update_data['remarks']) {
                echo json_encode(get_error_array(ESTABLISHMENT_REMARK_MESSAGE));
                return false;
            }
            ...
            $this->utility_lib->send_sms_and_email_for_app_reject($ex_data['user_id'], VALUE_EIGHT, VALUE_TEN, $textile_id);
    ...
    function get_textile_data_by_textile_id() {
        try {
            ...
            $this->db->trans_start();
            $textile_data = $this->utility_model->get_by_id_with_applicant_name('textile_id', $textile_id, 'textile');
            if (empty($textile_data)) {
                echo json_encode(get_error_array(INVALID_ACCESS_MESSAGE));
                return false;
            }
            if ($is_fb_details == VALUE_ONE || $is_fb_details == VALUE_TWO) {
                $fb_data = $this->utility_model->get_result_data_by_id('module_type', VALUE_TEN, 'fees_bifurcation', 'module_id', $textile_id);
                if ($is_fb_details == VALUE_TWO) {
                    $this->load->model('payment_model');
                    $ph_data = $this->payment_model->get_payment_history(VALUE_TEN, $textile_id);
                }
            }
            $this->db->trans_complete();
            if ($this->db->trans_status() === FALSE) {
    ...
    function generate_excel() {
        try {
            ...
            $session_district = get_from_session('temp_district_for_eodbsws_admin');
            $this->db->trans_start();
            $excel_data = $this->textile_model->get_records_for_excel($session_district);
            $this->db->trans_complete();
            if ($this->db->trans_status() === false) {
                print_r(INVALID_ACCESS_MESSAGE);
                return;
            ...
            header('Content-Disposition: attachment; filename=Textile_Report_' . date('Y-m-d H:i:s') . '.csv');
            $output = fopen("php://output", "w");
            fputcsv($output, array('Application Number', 'District', 'Entity / Establishment Type', 'Applicant Name', 'Mobile Number', 'Enterprise Name',
                'Office Address', 'Submitted On', 'Status', 'Query Status', 'Appr./Rej. By', 'Appr./Rej. datetime', 'Remarks'));
            if (!empty($excel_data)) {
                ...
                foreach ($excel_data as $list) {
                    $prefix = isset($prefix_module_array[VALUE_TEN]) ? $prefix_module_array[VALUE_TEN] : '';
                    $list['textile_id'] = generate_registration_number($prefix, $list['textile_id']);
