...
class Rii extends CI_Controller {
    ...
    function get_rii_data() {
        try {
            ...
            $this->db->trans_start();
            $success_array['rii_data'] = $this->rii_model->get_all_rii_list($start, $length, $search_district, $search_entity_establishment_type, $search_logged_user_detail, $search_applicant_detail, $search_app_timing, $search_status, $search_query_status);
            $success_array['recordsTotal'] = $this->rii_model->get_total_count_of_records($search_district);
            if (($search_district != '' && (is_admin() || is_view_all_district_user())) || $search_entity_establishment_type || $search_logged_user_detail != '' || $search_applicant_detail != '' || $search_app_timing != '' || $search_status != '' || $search_query_status != '') {
                $success_array['recordsFiltered'] = $this->rii_model->get_filter_count_of_records($search_district, $search_entity_establishment_type, $search_logged_user_detail, $search_applicant_detail, $search_app_timing, $search_status, $search_query_status);
            } else {
                $success_array['recordsFiltered'] = $success_array['recordsTotal'];
            }
            $success_array['query_movements'] = $this->utility_lib->get_query_movement_string($success_array['rii_data'], VALUE_FOURTYNINE);
    ...
    function get_rii_data_by_id() {
        try {
            ...
            $this->db->trans_start();
            $rii_data = $this->rii_model->get_rii_by_id($rii_id);
            if (empty($rii_data)) {
                echo json_encode(get_error_array(INVALID_USER_MESSAGE));
                return false;
            }
    ...
    function _update_image($user_id, $rii_id, &$rii_data, $is_exists_doc, $post_filename, $db_field_name) {
        ...
        if (!$temp_existing_doc_name) {
            ...
            $upload_doc_path = $module_path . DIRECTORY_SEPARATOR . $rii_id;
...
//            }
    ...
    function delete_upload_file_for_rii() {
        try {
            if (!is_ajax()) {
                header("Location:" . base_url() . "login");
                return false;
            ...
            if (!is_post() || $user_id == NULL || !$user_id || $rii_id == NULL || !$rii_id || $dbFileNameField == NULL || !$dbFileNameField) {
                echo json_encode(get_error_array('Invalid Access'));
                return false;
            }
            $this->db->trans_start();
            $existing_application_data = $this->rii_model->get_rii_by_id($rii_id);
            if (empty($existing_application_data)) {
                echo json_encode(get_error_array(INVALID_ACCESS_MESSAGE));
                return;
            }
            //document_file
            move_image($dbFileNameField, $existing_application_data, "documents/rii/$rii_id", 'garbage');

            $application_data = array();
            $application_data[$dbFileNameField] = '';
            $application_data['updated_by'] = $user_id;
            $application_data['updated_time'] = date('Y-m-d H:i:s');
            $this->rii_model->update_rii($rii_id, $application_data);
            $this->db->trans_complete();
            if ($this->db->trans_status() === FALSE) {
                echo json_encode(get_error_array(DATABASE_ERROR_MESSAGE));
                return;
            ...
            $success_array['upload_file_for_rii'] = $application_data;
            echo json_encode($success_array);
        } catch (\Exception $e) {
            echo json_encode(get_error_array($e->getMessage()));
            return false;
    ...
    function approve_for_rii() {
        try {
            ...
            $user_id = get_from_session('temp_id_for_eodbsws_admin');
            $rii_id = get_from_post('rii_id');
            if (!is_post() || $rii_id == NULL || !$rii_id || $user_id == NULL || !$user_id) {
                echo json_encode(get_error_array(INVALID_ACCESS_MESSAGE));
                return false;
            }
            $this->db->trans_start();
            ...
            $rii_data['updated_by'] = $user_id;
            $rii_data['updated_time'] = date('Y-m-d H:i:s');
            $this->rii_model->update_rii($rii_id, $rii_data);
            $this->db->trans_complete();
            if ($this->db->trans_status() === FALSE) {
                echo json_encode(get_error_array(DATABASE_ERROR_MESSAGE));
                return;
    ...
    function get_rii_data_by_rii_id() {
        try {
            ...
            $this->db->trans_start();
            $rii_data = $this->utility_model->get_by_id_with_applicant_name('rii_id', $rii_id, 'rii');
            if (empty($rii_data)) {
                echo json_encode(get_error_array(INVALID_ACCESS_MESSAGE));
                return false;
            }
            if ($is_fb_details == VALUE_ONE || $is_fb_details == VALUE_TWO) {
                $fb_data = $this->utility_model->get_result_data_by_id('module_type', VALUE_FOURTYNINE, 'fees_bifurcation', 'module_id', $rii_id);
                if ($is_fb_details == VALUE_TWO) {
                    $this->load->model('payment_model');
                    $ph_data = $this->payment_model->get_payment_history(VALUE_FOURTYNINE, $rii_id);
                }
            }
            $this->db->trans_complete();
            if ($this->db->trans_status() === FALSE) {
    ...
    function remove_challan() {
        try {
            ...
            $file_path = 'documents' . DIRECTORY_SEPARATOR . 'rii' . DIRECTORY_SEPARATOR . $ex_est_data['challan'];
            if (file_exists($file_path)) {
                unlink($file_path);
            }
            $this->utility_model->update_data('rii_id', $rii_id, 'rii', array('status' => VALUE_TWO, 'challan' => '', 'updated_by' => $session_user_id, 'updated_time' => date('Y-m-d H:i:s')));
            $success_array = get_success_array();
    ...
    function upload_challan() {
        try {
            ...
            $payment_type = get_from_post('payment_type_for_rii_upload_challan');
            if ($payment_type != VALUE_ONE && $payment_type != VALUE_TWO) {
                echo json_encode(get_error_array(ONE_PAYMENT_OPTION_MESSAGE));
                return false;
            }
            $rii_data = array();
            if ($_FILES['challan_for_rii_upload_challan']['name'] != '') {
                ...
                $this->load->library('upload');
                $temp_filename = str_replace('_', '', $_FILES['challan_for_rii_upload_challan']['name']);
                $filename = 'challan_dd_po_' . (rand(100000000, 999999999)) . time() . '.' . pathinfo($temp_filename, PATHINFO_EXTENSION);
                //Change file name
                $final_path = $main_path . DIRECTORY_SEPARATOR . $filename;
                if (!move_uploaded_file($_FILES['challan_for_rii_upload_challan']['tmp_name'], $final_path)) {
                    echo json_encode(get_error_array(DOCUMENT_NOT_UPLOAD_MESSAGE));
                    return;
                }
                $rii_data['status'] = VALUE_THREE;
                $rii_data['challan'] = $filename;
                $rii_data['challan_updated_date'] = date('Y-m-d H:i:s');
            }
            $rii_data['payment_type'] = $payment_type;
            $rii_data['updated_by'] = $user_id;
            $rii_data['updated_time'] = date('Y-m-d H:i:s');
            $rii_data['total_fees'] = VALUE_ZERO;
            $this->db->trans_start();

            if ($payment_type == VALUE_ONE || $payment_type == VALUE_TWO) {
                $error_message = $this->utility_lib->update_fees_bifurcation_details(VALUE_FOURTYNINE, $rii_id, $user_id, $rii_data);
                if ($error_message != '') {
                ...
                $this->utility_model->update_data('module_type', VALUE_FOURTYNINE, 'fees_bifurcation', $update_data, 'module_id', $rii_id);
            }
            ...
            $success_array['total_fees'] = $rii_data['total_fees'];
            echo json_encode($success_array);
        } catch (\Exception $e) {
            echo json_encode(get_error_array($e->getMessage()));
            return false;
    ...
    function approve_application() {
        try {
            ...
            $session_user_id = get_from_session('temp_id_for_eodbsws_admin');
            $rii_id = get_from_post('rii_id_for_rii_approve');
            if (!is_post() || $session_user_id == NULL || !$session_user_id || !$rii_id || $rii_id == NULL) {
                echo json_encode(get_error_array(INVALID_ACCESS_MESSAGE));
                return false;
            }
            $update_data = array();
            $update_data['registration_number'] = "LE/LI/DMN/RII-" . get_from_post('registration_number_for_rii_approve') . "/" . date('Y');
            $update_data['valid_upto'] = get_from_post('valid_upto_for_rii_approve');
            $update_data['remarks'] = get_from_post('remarks_for_rii_approve');
            ...
            $existing_rii_data = $this->utility_model->check_registration_number('registration_number', $update_data['registration_number'], 'rii');
            if (is_array($existing_rii_data)) {
                if (count(array($existing_rii_data)) > 0) {
            ...
            $this->db->trans_start();
            $ex_data = $this->utility_model->get_by_id('rii_id', $rii_id, 'rii');
            if (empty($ex_data)) {
                echo json_encode(get_error_array(INVALID_ACCESS_MESSAGE));
                return false;
            }
            $update_data['processing_days'] = $this->utility_lib->calculate_processing_days(VALUE_FOURTYNINE, $ex_data['submitted_datetime']);
            $update_data['status'] = VALUE_FIVE;
            $update_data['status_datetime'] = date('Y-m-d H:i:s');
            $update_data['updated_by'] = $session_user_id;
            $update_data['updated_time'] = date('Y-m-d H:i:s');
            $this->utility_model->update_data('rii_id', $rii_id, 'rii', $update_data);

            $this->utility_lib->send_sms_and_email_for_app_approve($ex_data['user_id'], VALUE_SEVEN, VALUE_FOURTYNINE, $rii_id);
            $this->db->trans_complete();
            if ($this->db->trans_status() === FALSE) {
                echo json_encode(get_error_array(DATABASE_ERROR_MESSAGE));
                return;
    ...
    function reject_application() {
        try {
            ...
            $session_user_id = get_from_session('temp_id_for_eodbsws_admin');
            $rii_id = get_from_post('rii_id_for_rii_reject');
            if (!is_post() || $session_user_id == NULL || !$session_user_id || !$rii_id || $rii_id == NULL) {
                echo json_encode(get_error_array(INVALID_ACCESS_MESSAGE));
                return false;
            }
            $update_data = array();
            $update_data['remarks'] = get_from_post('remarks_for_rii_reject');
            if (!$update_data['remarks']) {
                echo json_encode(get_error_array(ESTABLISHMENT_REMARK_MESSAGE));
                return false;
            }
            ...
            $this->utility_lib->send_sms_and_email_for_app_reject($ex_data['user_id'], VALUE_EIGHT, VALUE_FOURTYNINE, $rii_id);
            $this->db->trans_complete();
            if ($this->db->trans_status() === FALSE) {
                echo json_encode(get_error_array(DATABASE_ERROR_MESSAGE));
                return;
    ...
    function generate_certificate() {
        try {
            ...
            $mpdf->WriteHTML($this->load->view('rii/certificate', $data, TRUE));
            $mpdf->Output('RII_certificate_' . time() . '.pdf', 'D');
        } catch (\Exception $e) {
            print_r($e->getMessage());
            return false;
        }
    ...
    function generate_excel() {
        try {
            ...
            $session_district = get_from_session('temp_district_for_eodbsws_admin');
            $this->db->trans_start();
            $excel_data = $this->rii_model->get_records_for_excel($session_district);
            $this->db->trans_complete();
            if ($this->db->trans_status() === false) {
                print_r(INVALID_ACCESS_MESSAGE);
                return;
            ...
            header('Content-Disposition: attachment; filename=Rii_Report_' . date('Y-m-d H:i:s') . '.csv');
            $output = fopen("php://output", "w");
            fputcsv($output, array('Application Number', 'District', 'Entity / Establishment Type', 'Applicant Name', 'Mobile Number', 'Name of User/ Premises', 'Address', 'Submitted On', 'Status', 'Query Status', 'Appr./Rej. By', 'Appr./Rej. datetime', 'Remarks'));
            if (!empty($excel_data)) {
                ...
                foreach ($excel_data as $list) {
                    $prefix = isset($prefix_module_array[VALUE_FOURTYNINE]) ? $prefix_module_array[VALUE_FOURTYNINE] : '';
                    $list['rii_id'] = generate_registration_number($prefix, $list['rii_id']);
...
/*
 * EOF: ./application/controller/RII.php
 */