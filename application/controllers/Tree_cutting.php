...
class Tree_cutting extends CI_Controller {
    ...
    function get_tree_cutting_data() {
        try {
            ...
            $this->db->trans_start();
            $success_array['tree_cutting_data'] = $this->tree_cutting_model->get_all_tree_cutting_list($start, $length, $search_district, $search_eet, $search_logged_user_detail, $search_applicant_detail, $search_app_timing, $search_status, $search_query_status);
            $success_array['recordsTotal'] = $this->tree_cutting_model->get_total_count_of_records($search_district);
            if (($search_district != '' && (is_admin() || is_view_all_district_user())) || $search_eet || $search_logged_user_detail != '' || $search_applicant_detail != '' || $search_app_timing != '' || $search_status != '' || $search_query_status != '') {
                $success_array['recordsFiltered'] = $this->tree_cutting_model->get_filter_count_of_records($search_district, $search_eet, $search_logged_user_detail, $search_applicant_detail, $search_app_timing, $search_status, $search_query_status);
            } else {
                $success_array['recordsFiltered'] = $success_array['recordsTotal'];
            }
            $success_array['query_movements'] = $this->utility_lib->get_query_movement_string($success_array['tree_cutting_data'], VALUE_FIFTYNINE);
    ...
    function remove_challan() {
        try {
            ...
            $file_path = 'documents' . DIRECTORY_SEPARATOR . 'tree_cutting' . DIRECTORY_SEPARATOR . $ex_est_data['challan'];
            if (file_exists($file_path)) {
                unlink($file_path);
            }
            $this->utility_model->update_data('tree_cutting_id', $tree_cutting_id, 'tree_cutting', array('status' => VALUE_TWO, 'challan' => '', 'updated_by' => $session_user_id, 'updated_time' => date('Y-m-d H:i:s')));
            $success_array = get_success_array();
    ...
    function upload_challan() {
        try {
            ...
            $payment_type = get_from_post('payment_type_for_tree_cutting_upload_challan');
            ...
            if ($_FILES['challan_for_tree_cutting_upload_challan']['name'] != '') {
                ...
                $temp_filename = str_replace('_', '', $_FILES['challan_for_tree_cutting_upload_challan']['name']);
                ...
                if (!move_uploaded_file($_FILES['challan_for_tree_cutting_upload_challan']['tmp_name'], $final_path)) {
                    echo json_encode(get_error_array(DOCUMENT_NOT_UPLOAD_MESSAGE));
                    return;
                }
                $tree_cutting_data['challan'] = $filename;
                $tree_cutting_data['challan_updated_date'] = date('Y-m-d H:i:s');
            }
            $tree_cutting_data['status'] = VALUE_THREE;
            if ($payment_type == VALUE_THREE) {
                $tree_cutting_data['status'] = VALUE_NINE;
            }
            $tree_cutting_data['payment_type'] = $payment_type;
            $tree_cutting_data['updated_by'] = $user_id;
            $tree_cutting_data['updated_time'] = date('Y-m-d H:i:s');
            $tree_cutting_data['total_fees'] = VALUE_ZERO;
            $this->db->trans_start();

            if ($payment_type == VALUE_ONE || $payment_type == VALUE_TWO) {
                $error_message = $this->utility_lib->update_fees_bifurcation_details(VALUE_FIFTYNINE, $tree_cutting_id, $user_id, $tree_cutting_data);
                if ($error_message != '') {
                ...
                $this->utility_model->update_data('module_type', VALUE_FIFTYNINE, 'fees_bifurcation', $update_data, 'module_id', $tree_cutting_id);
            }
            ...
            $success_array['total_fees'] = $tree_cutting_data['total_fees'];
            echo json_encode($success_array);
        } catch (\Exception $e) {
            echo json_encode(get_error_array($e->getMessage()));
            return false;
    ...
    function approve_application() {
        try {
            ...
            $session_user_id = get_from_session('temp_id_for_eodbsws_admin');
            $tree_cutting_id = get_from_post('tree_cutting_id_for_tree_cutting_approve');
            if (!is_post() || $session_user_id == NULL || !$session_user_id || !$tree_cutting_id || $tree_cutting_id == NULL) {
                echo json_encode(get_error_array(INVALID_ACCESS_MESSAGE));
                return false;
            }
            $update_data = array();
            $update_data['remarks'] = get_from_post('remarks_for_tree_cutting_approve');
            if (!$update_data['remarks']) {
                echo json_encode(get_error_array(REMARKS_MESSAGE));
                return false;
            }
            $this->db->trans_start();
            $ex_data = $this->utility_model->get_by_id('tree_cutting_id', $tree_cutting_id, 'tree_cutting');
            if (empty($ex_data)) {
                echo json_encode(get_error_array(INVALID_ACCESS_MESSAGE));
                return false;
            }
            if ($_FILES['certificate_file_for_tree_cutting_approve']['name'] == '') {
                echo json_encode(get_error_array(UPLOAD_DOC_MESSAGE));
                return;
            }
            $evidence_size = $_FILES['certificate_file_for_tree_cutting_approve']['size'];
            if ($evidence_size == 0) {
            ...
            $ad_path = $main_path . DIRECTORY_SEPARATOR . 'forest';
            if (!is_dir($ad_path)) {
                mkdir($ad_path);
                chmod("$ad_path", 0755);
            ...
            $this->load->library('upload');
            $temp_filename = str_replace('_', '', $_FILES['certificate_file_for_tree_cutting_approve']['name']);
            $filename = 'tree_cuttingc_' . (rand(100000000, 999999999)) . time() . '.' . pathinfo($temp_filename, PATHINFO_EXTENSION);
            //Change file name
            $final_path = $ad_path . DIRECTORY_SEPARATOR . $filename;
            if (!move_uploaded_file($_FILES['certificate_file_for_tree_cutting_approve']['tmp_name'], $final_path)) {
                echo json_encode(get_error_array(DOCUMENT_NOT_UPLOAD_MESSAGE));
                return;
            }
            $update_data['certificate_file'] = $filename;
            $temp_fc_filename = 'temp_tree_cuttingfc_' . (rand(100000000, 999999999)) . time() . '.pdf';
            error_reporting(E_ERROR);
            $data = array('tree_cutting_data' => $ex_data);
            $mpdf = new \Mpdf\Mpdf(['mode' => 'utf-8', 'format' => 'A4']);
            $mpdf->WriteHTML($this->load->view('tree_cutting/qr_barcode', $data, TRUE));
            ...
            $final_certificate_filename = 'tree_cuttingfc_' . (rand(100000000, 999999999)) . time() . '.pdf';
            ...
            $update_data['processing_days'] = $this->utility_lib->calculate_processing_days(VALUE_FIFTYNINE, $ex_data['submitted_datetime']);
            $update_data['status'] = VALUE_FIVE;
            $update_data['status_datetime'] = date('Y-m-d H:i:s');
            $update_data['updated_by'] = $session_user_id;
            $update_data['updated_time'] = date('Y-m-d H:i:s');
            $this->utility_model->update_data('tree_cutting_id', $tree_cutting_id, 'tree_cutting', $update_data);

            $this->utility_lib->send_sms_and_email_for_app_approve($ex_data['user_id'], VALUE_SEVEN, VALUE_FIFTYNINE, $tree_cutting_id);
    ...
    function reject_application() {
        try {
            ...
            $session_user_id = get_from_session('temp_id_for_eodbsws_admin');
            $tree_cutting_id = get_from_post('tree_cutting_id_for_tree_cutting_reject');
            if (!is_post() || $session_user_id == NULL || !$session_user_id || !$tree_cutting_id || $tree_cutting_id == NULL) {
                echo json_encode(get_error_array(INVALID_ACCESS_MESSAGE));
                return false;
            }
            $update_data = array();
            $update_data['remarks'] = get_from_post('remarks_for_tree_cutting_reject');
            if (!$update_data['remarks']) {
                echo json_encode(get_error_array(REMARKS_MESSAGE));
                return false;
            }
            ...
            $this->utility_lib->send_sms_and_email_for_app_reject($ex_data['user_id'], VALUE_EIGHT, VALUE_FIFTYNINE, $tree_cutting_id);
    ...
    function get_tree_cutting_data_by_tree_cutting_id() {
        try {
            ...
            $this->db->trans_start();
            $tree_cutting_data = $this->utility_model->get_by_id_with_applicant_name('tree_cutting_id', $tree_cutting_id, 'tree_cutting');
            if (empty($tree_cutting_data)) {
                echo json_encode(get_error_array(INVALID_ACCESS_MESSAGE));
                return false;
            }
            if ($is_fb_details == VALUE_ONE || $is_fb_details == VALUE_TWO) {
                $fb_data = $this->utility_model->get_result_data_by_id('module_type', VALUE_FIFTYNINE, 'fees_bifurcation', 'module_id', $tree_cutting_id);
                if ($is_fb_details == VALUE_TWO) {
                    $this->load->model('payment_model');
                    $ph_data = $this->payment_model->get_payment_history(VALUE_FIFTYNINE, $tree_cutting_id);
                }
                if ($tree_cutting_data['status'] != VALUE_FOUR && $tree_cutting_data['status'] != VALUE_FIVE &&
    ...
    function generate_tree_cutting_excel() {
        try {
            $user_id = get_from_session('temp_id_for_eodbsws_admin');
            ...
            $session_district = get_from_session('temp_district_for_eodbsws_admin');
            $this->db->trans_start();
            $excel_data = $this->tree_cutting_model->get_records_for_excel($session_district);
            $this->db->trans_complete();
            if ($this->db->trans_status() === false) {
                print_r(INVALID_ACCESS_MESSAGE);
                return;
            ...
            header('Content-Disposition: attachment; filename=Tree_cutting_Report_' . date('Y-m-d H:i:s') . '.csv');
            ...
            fputcsv($output, array('Application Number', 'District', 'Village / DMC Ward / SMC Ward',
                ...
                'Applicant Name', 'Applicant Address', 'Applicant Mobile Number', 'Owner Name', 'Owner Address',
            ...
            if (!empty($excel_data)) {
                ...
                foreach ($excel_data as $list) {
                    ...
                    $prefix = isset($prefix_module_array[VALUE_FIFTYNINE]) ? $prefix_module_array[VALUE_FIFTYNINE] : '';
                    $list['tree_cutting_id'] = generate_registration_number($prefix, $list['tree_cutting_id']);
