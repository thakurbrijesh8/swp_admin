...
class Msme extends CI_Controller {
    ...
    function get_msme_data() {
        try {
            ...
            $this->db->trans_start();
            $success_array['msme_data'] = $this->msme_model->get_all_msme_list($start, $length, $search_district, $search_entity_establishment_type, $search_logged_user_detail, $search_applicant_detail, $search_app_timing, $search_status, $search_query_status);
            ...
            if (($search_district != '' && (is_admin() || is_view_all_district_user())) || $search_entity_establishment_type || $search_logged_user_detail != '' || $search_applicant_detail != '' || $search_app_timing != '' || $search_status != '' || $search_query_status != '') {
                $success_array['recordsFiltered'] = $this->msme_model->get_filter_count_of_records($search_district, $search_entity_establishment_type, $search_logged_user_detail, $search_applicant_detail, $search_app_timing, $search_status, $search_query_status);
            } else {
                $success_array['recordsFiltered'] = $success_array['recordsTotal'];
            }
            $success_array['query_movements'] = $this->utility_lib->get_query_movement_string($success_array['msme_data'], VALUE_NINE);
    ...
    function _check_validation_for_msme($msme_data) {
        ...
        if (!$msme_data['entity_establishment_type']) {
    ...
    function remove_challan() {
        try {
            ...
            $this->utility_model->update_data('msme_id', $msme_id, 'msme', array('status' => VALUE_TWO, 'challan' => '', 'updated_by' => $session_user_id, 'updated_time' => date('Y-m-d H:i:s')));
            $success_array = get_success_array();
    ...
    function upload_challan() {
        try {
            ...
            if ($payment_type == VALUE_THREE) {
                $msme_data['status'] = VALUE_NINE;
            }
            $msme_data['payment_type'] = $payment_type;
            $msme_data['updated_by'] = $user_id;
            $msme_data['updated_time'] = date('Y-m-d H:i:s');
            $msme_data['total_fees'] = VALUE_ZERO;
            $this->db->trans_start();

            if ($payment_type == VALUE_ONE || $payment_type == VALUE_TWO) {
                $error_message = $this->utility_lib->update_fees_bifurcation_details(VALUE_NINE, $msme_id, $user_id, $msme_data);
                if ($error_message != '') {
                ...
                $this->utility_model->update_data('module_type', VALUE_NINE, 'fees_bifurcation', $update_data, 'module_id', $msme_id);
            }
            ...
            $success_array['total_fees'] = $msme_data['total_fees'];
            echo json_encode($success_array);
        } catch (\Exception $e) {
            echo json_encode(get_error_array($e->getMessage()));
            return false;
    ...
    function approve_application() {
        try {
            ...
            $session_user_id = get_from_session('temp_id_for_eodbsws_admin');
            $msme_id = get_from_post('msme_id_for_msme_approve');
            if (!is_post() || $session_user_id == NULL || !$session_user_id || !$msme_id || $msme_id == NULL) {
                echo json_encode(get_error_array(INVALID_ACCESS_MESSAGE));
                return false;
            }
            ...
            $this->db->trans_start();
            $ex_data = $this->utility_model->get_by_id('msme_id', $msme_id, 'msme');
            if (empty($ex_data)) {
                echo json_encode(get_error_array(INVALID_ACCESS_MESSAGE));
                return false;
            }
            if ($_FILES['certificate_file_for_msme_approve']['name'] == '') {
                echo json_encode(get_error_array(UPLOAD_DOC_MESSAGE));
                return;
            }
            $evidence_size = $_FILES['certificate_file_for_msme_approve']['size'];
            if ($evidence_size == 0) {
            ...
            $this->load->library('upload');
            $temp_filename = str_replace('_', '', $_FILES['certificate_file_for_msme_approve']['name']);
            $filename = 'msmec_' . (rand(100000000, 999999999)) . time() . '.' . pathinfo($temp_filename, PATHINFO_EXTENSION);
            //Change file name
            $final_path = $ad_path . DIRECTORY_SEPARATOR . $filename;
            if (!move_uploaded_file($_FILES['certificate_file_for_msme_approve']['tmp_name'], $final_path)) {
                echo json_encode(get_error_array(DOCUMENT_NOT_UPLOAD_MESSAGE));
                return;
            }
            $update_data['certificate_file'] = $filename;
            $temp_fc_filename = 'temp_msmefc_' . (rand(100000000, 999999999)) . time() . '.pdf';
            error_reporting(E_ERROR);
            $data = array('msme_data' => $ex_data);
            $mpdf = new \Mpdf\Mpdf(['mode' => 'utf-8', 'format' => 'A4']);
            $mpdf->WriteHTML($this->load->view('msme/qr_barcode', $data, TRUE));
            ...
            $final_certificate_filename = 'msmefc_' . (rand(100000000, 999999999)) . time() . '.pdf';
            ...
            $this->utility_model->update_data('msme_id', $msme_id, 'msme', $update_data);

            $this->utility_lib->send_sms_and_email_for_app_approve($ex_data['user_id'], VALUE_SEVEN, VALUE_NINE, $msme_id);
    ...
    function reject_application() {
        try {
            ...
            $session_user_id = get_from_session('temp_id_for_eodbsws_admin');
            $msme_id = get_from_post('msme_id_for_msme_reject');
            if (!is_post() || $session_user_id == NULL || !$session_user_id || !$msme_id || $msme_id == NULL) {
                echo json_encode(get_error_array(INVALID_ACCESS_MESSAGE));
                return false;
            }
            ...
            $this->utility_lib->send_sms_and_email_for_app_reject($ex_data['user_id'], VALUE_EIGHT, VALUE_NINE, $msme_id);
    ...
    function get_msme_data_by_msme_id() {
        try {
            ...
            $this->db->trans_start();
            $msme_data = $this->utility_model->get_by_id_with_applicant_name('msme_id', $msme_id, 'msme');
            if (empty($msme_data)) {
                echo json_encode(get_error_array(INVALID_ACCESS_MESSAGE));
                return false;
            }
            if ($is_fb_details == VALUE_ONE || $is_fb_details == VALUE_TWO) {
                $fb_data = $this->utility_model->get_result_data_by_id('module_type', VALUE_NINE, 'fees_bifurcation', 'module_id', $msme_id);
                if ($is_fb_details == VALUE_TWO) {
                    $this->load->model('payment_model');
                    $ph_data = $this->payment_model->get_payment_history(VALUE_NINE, $msme_id);
                }
            }
            $this->db->trans_complete();
            if ($this->db->trans_status() === FALSE) {
        ...
        try {
            ...
            if (!empty($excel_data)) {
                ...
                foreach ($excel_data as $list) {
                    ...
                    $list['msme_id'] = generate_registration_number($prefix, $list['msme_id']);
