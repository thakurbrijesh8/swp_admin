...
class Travelagent extends CI_Controller {
    ...
    function get_travelagent_data() {
        try {
            ...
            $this->db->trans_start();
            $success_array['travelagent_data'] = $this->travelagent_model->get_all_travelagent_list($start, $length, $search_district, $search_entity_establishment_type, $search_logged_user_detail, $search_applicant_detail, $search_app_timing, $search_status, $search_query_status);
            $success_array['recordsTotal'] = $this->travelagent_model->get_total_count_of_records($search_district);
            ...
            if (($search_district != '' && (is_admin() || is_view_all_district_user())) || $search_entity_establishment_type || $search_logged_user_detail != '' || $search_applicant_detail != '' || $search_app_timing != '' || $search_status != '' || $search_query_status != '') {
                $success_array['recordsFiltered'] = $this->travelagent_model->get_filter_count_of_records($search_district, $search_entity_establishment_type, $search_logged_user_detail, $search_applicant_detail, $search_app_timing, $search_status, $search_query_status);
            } else {
                $success_array['recordsFiltered'] = $success_array['recordsTotal'];
            }
            $success_array['query_movements'] = $this->utility_lib->get_query_movement_string($success_array['travelagent_data'], VALUE_NINETEEN);
    ...
    function get_travelagent_data_by_id() {
        try {
            ...
            $this->db->trans_start();
            $travelagent_data = $this->travelagent_model->get_travelagent_by_id($travelagent_id);
    ...
    function get_travelagent_data_by_travelagent_id() {
        try {
            ...
            $this->db->trans_start();
            $travelagent_data = $this->utility_model->get_by_id_with_applicant_name('travelagent_id', $travelagent_id, 'travelagent');
            if (empty($travelagent_data)) {
                echo json_encode(get_error_array(INVALID_ACCESS_MESSAGE));
                return false;
            }
            if ($is_fb_details == VALUE_ONE || $is_fb_details == VALUE_TWO) {
                $fb_data = $this->utility_model->get_result_data_by_id('module_type', VALUE_NINETEEN, 'fees_bifurcation', 'module_id', $travelagent_id);
                if ($is_fb_details == VALUE_TWO) {
                    $this->load->model('payment_model');
                    $ph_data = $this->payment_model->get_payment_history(VALUE_NINETEEN, $travelagent_id);
                }
            }
            $this->db->trans_complete();
            if ($this->db->trans_status() === FALSE) {
    ...
    function remove_challan() {
        try {
            ...
            $this->utility_model->update_data('travelagent_id', $travelagent_id, 'travelagent', array('status' => VALUE_TWO, 'challan' => '', 'updated_by' => $session_user_id, 'updated_time' => date('Y-m-d H:i:s')));
            $success_array = get_success_array();
            $success_array['message'] = DOCUMENT_REMOVED_MESSAGE;
            echo json_encode($success_array);
        } catch (\Exception $e) {
    ...
    function upload_challan() {
        try {
            ...
            $payment_type = get_from_post('payment_type_for_travelagent_upload_challan');
            ...
            $travelagent_data = array();
            if ($_FILES['challan_for_travelagent_upload_challan']['name'] != '') {
                ...
                $temp_filename = str_replace('_', '', $_FILES['challan_for_travelagent_upload_challan']['name']);
                $filename = 'challan_dd_po_' . (rand(100000000, 999999999)) . time() . '.' . pathinfo($temp_filename, PATHINFO_EXTENSION);
                //Change file name
                $final_path = $main_path . DIRECTORY_SEPARATOR . $filename;
                if (!move_uploaded_file($_FILES['challan_for_travelagent_upload_challan']['tmp_name'], $final_path)) {
                    echo json_encode(get_error_array(DOCUMENT_NOT_UPLOAD_MESSAGE));
                    return;
                }
                $travelagent_data['challan'] = $filename;
                $travelagent_data['challan_updated_date'] = date('Y-m-d H:i:s');
            }
            $travelagent_data['status'] = VALUE_THREE;
            if ($payment_type == VALUE_THREE) {
                $travelagent_data['status'] = VALUE_NINE;
            }
            $travelagent_data['payment_type'] = $payment_type;
            $travelagent_data['updated_by'] = $user_id;
            $travelagent_data['updated_time'] = date('Y-m-d H:i:s');
            $travelagent_data['total_fees'] = VALUE_ZERO;
            $this->db->trans_start();

            if ($payment_type == VALUE_ONE || $payment_type == VALUE_TWO) {
                $error_message = $this->utility_lib->update_fees_bifurcation_details(VALUE_NINETEEN, $travelagent_id, $user_id, $travelagent_data);
                if ($error_message != '') {
                ...
                $this->utility_model->update_data('module_type', VALUE_NINETEEN, 'fees_bifurcation', $update_data, 'module_id', $travelagent_id);
            }
            ...
            $success_array['total_fees'] = $travelagent_data['total_fees'];
            echo json_encode($success_array);
        } catch (\Exception $e) {
            echo json_encode(get_error_array($e->getMessage()));
            return false;
    ...
    function approve_application() {
        try {
            ...
            $session_user_id = get_from_session('temp_id_for_eodbsws_admin');
            $travelagent_id = get_from_post('travelagent_id_for_travelagent_approve');
            if (!is_post() || $session_user_id == NULL || !$session_user_id || !$travelagent_id || $travelagent_id == NULL) {
                echo json_encode(get_error_array(INVALID_ACCESS_MESSAGE));
                return false;
            }
            $update_data = array();
            $update_data['registration_number'] = get_from_post('registration_number_for_travelagent_approve');
            $update_data['valid_upto'] = get_from_post('valid_upto_for_travelagent_approve');
            $update_data['remarks'] = get_from_post('remarks_for_travelagent_approve');
            ...
            $update_data['last_valid_upto'] = convert_to_mysql_date_format($update_data['valid_upto']);
            $this->db->trans_start();
            $ex_data = $this->utility_model->get_by_id('travelagent_id', $travelagent_id, 'travelagent');
            if (empty($ex_data)) {
                echo json_encode(get_error_array(INVALID_ACCESS_MESSAGE));
                return false;
            }
            $update_data['processing_days'] = $this->utility_lib->calculate_processing_days(VALUE_NINETEEN, $ex_data['submitted_datetime']);
            $update_data['status'] = VALUE_FIVE;
            $update_data['status_datetime'] = date('Y-m-d H:i:s');
            $update_data['updated_by'] = $session_user_id;
            $update_data['updated_time'] = date('Y-m-d H:i:s');
            $this->utility_model->update_data('travelagent_id', $travelagent_id, 'travelagent', $update_data);

            $this->utility_lib->send_sms_and_email_for_app_approve($ex_data['user_id'], VALUE_SEVEN, VALUE_NINETEEN, $travelagent_id);
    ...
    function reject_application() {
        try {
            ...
            $session_user_id = get_from_session('temp_id_for_eodbsws_admin');
            $travelagent_id = get_from_post('travelagent_id_for_travelagent_reject');
            if (!is_post() || $session_user_id == NULL || !$session_user_id || !$travelagent_id || $travelagent_id == NULL) {
                echo json_encode(get_error_array(INVALID_ACCESS_MESSAGE));
                return false;
            }
            $update_data = array();
            $update_data['remarks'] = get_from_post('remarks_for_travelagent_reject');
            if (!$update_data['remarks']) {
                echo json_encode(get_error_array(ESTABLISHMENT_REMARK_MESSAGE));
                return false;
            }
            ...
            $this->utility_lib->send_sms_and_email_for_app_reject($ex_data['user_id'], VALUE_EIGHT, VALUE_NINETEEN, $travelagent_id);
    ...
    function generate_certificate() {
        try {
            ...
            $data = array('travelagent_data' => $existing_travelagent_data);
            $mpdf = new \Mpdf\Mpdf(['mode' => 'utf-8', 'format' => 'Legal']);
            $mpdf->WriteHTML($this->load->view('travelagent/certificate', $data, TRUE));
            $mpdf->Output('Travelagent_certificate_' . time() . '.pdf', 'I');
        } catch (\Exception $e) {
            print_r($e->getMessage());
            return false;
        }
    ...
    function generate_excel() {
        try {
            ...
            $session_district = get_from_session('temp_district_for_eodbsws_admin');
            $this->db->trans_start();
            $excel_data = $this->travelagent_model->get_records_for_excel($session_district);
            $this->db->trans_complete();
            if ($this->db->trans_status() === false) {
                print_r(INVALID_ACCESS_MESSAGE);
                return;
            ...
            header('Content-Disposition: attachment; filename=Travel_Agency_Report_' . date('Y-m-d H:i:s') . '.csv');
            $output = fopen("php://output", "w");
            fputcsv($output, array('Application Number', 'District', 'Entity / Establishment Type', 'Applicant Name', 'Mobile Number', 'Name of Person',
                'Name of Travel Agency', 'Submitted On', 'Status', 'Query Status', 'Appr./Rej. By', 'Appr./Rej. datetime', 'Remarks'));
            if (!empty($excel_data)) {
                $taluka_array = $this->config->item('taluka_array');
                ...
                foreach ($excel_data as $list) {
                    $prefix = isset($prefix_module_array[VALUE_NINETEEN]) ? $prefix_module_array[VALUE_NINETEEN] : '';
                    $list['travelagent_id'] = generate_registration_number($prefix, $list['travelagent_id']);
