...
class Manufacturer_renewal extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('manufacturer_renewal_model');
    }
    ...
    function get_manufacturer_renewal_data() {
        try {
            ...
            $this->db->trans_start();
            $success_array['manufacturer_renewal_data'] = $this->manufacturer_renewal_model->get_all_manufacturer_renewal_list($start, $length, $search_district, $search_entity_establishment_type, $search_logged_user_detail, $search_applicant_detail, $search_app_timing, $search_status, $search_query_status);
            $success_array['recordsTotal'] = $this->manufacturer_renewal_model->get_total_count_of_records($search_district);
            ...
            if (($search_district != '' && (is_admin() || is_view_all_district_user())) || $search_entity_establishment_type || $search_logged_user_detail != '' || $search_applicant_detail != '' || $search_app_timing != '' || $search_status != '' || $search_query_status != '') {
                $success_array['recordsFiltered'] = $this->manufacturer_renewal_model->get_filter_count_of_records($search_district, $search_entity_establishment_type, $search_logged_user_detail, $search_applicant_detail, $search_app_timing, $search_status, $search_query_status);
            } else {
                $success_array['recordsFiltered'] = $success_array['recordsTotal'];
            }
            $success_array['query_movements'] = $this->utility_lib->get_query_movement_string($success_array['manufacturer_renewal_data'], VALUE_SIXTEEN);
    ...
    function get_manufacturer_data_by_id() {
        try {
            ...
            $this->db->trans_start();
            $manufacturer_data = $this->utility_model->get_by_id('admin_registration_number', $license_number, 'wm_manufacturer_renewal');
    ...
    function remove_document() {
        try {
            ...
            $this->utility_model->update_data('manufacturer_renewal_id', $manufacturer_renewal_id, 'wm_manufacturer_renewal', array($document_id => '', 'updated_by' => $session_user_id, 'updated_time' => date('Y-m-d H:i:s')));
    ...
    function get_manufacturer_renewal_data_by_manufacturer_renewal_id() {
        try {
            ...
            $this->db->trans_start();
            $manufacturer_renewal_data = $this->utility_model->get_by_id_with_applicant_name('manufacturer_renewal_id', $manufacturer_renewal_id, 'wm_manufacturer_renewal');
            if (empty($manufacturer_renewal_data)) {
                echo json_encode(get_error_array(INVALID_ACCESS_MESSAGE));
                return false;
            }
            if ($is_fb_details == VALUE_ONE || $is_fb_details == VALUE_TWO) {
                $fb_data = $this->utility_model->get_result_data_by_id('module_type', VALUE_SIXTEEN, 'fees_bifurcation', 'module_id', $manufacturer_renewal_id);
                if ($is_fb_details == VALUE_TWO) {
                    $this->load->model('payment_model');
                    $ph_data = $this->payment_model->get_payment_history(VALUE_SIXTEEN, $manufacturer_renewal_id);
                }
                if ($manufacturer_renewal_data['status'] != VALUE_FOUR && $manufacturer_renewal_data['status'] != VALUE_FIVE &&
    ...
    function remove_challan() {
        try {
            ...
            $this->utility_model->update_data('manufacturer_renewal_id', $manufacturer_renewal_id, 'wm_manufacturer_renewal', array('status' => VALUE_TWO, 'challan' => '', 'updated_by' => $session_user_id, 'updated_time' => date('Y-m-d H:i:s')));
            $success_array = get_success_array();
            $success_array['message'] = DOCUMENT_REMOVED_MESSAGE;
            echo json_encode($success_array);
        } catch (\Exception $e) {
    ...
    function upload_challan() {
        try {
            ...
            $payment_type = get_from_post('payment_type_for_manufacturer_renewal_upload_challan');
            ...
            if ($_FILES['challan_for_manufacturer_renewal_upload_challan']['name'] != '') {
                ...
                $temp_filename = str_replace('_', '', $_FILES['challan_for_manufacturer_renewal_upload_challan']['name']);
                $filename = 'challan_dd_po_' . (rand(100000000, 999999999)) . time() . '.' . pathinfo($temp_filename, PATHINFO_EXTENSION);
                //Change file name
                $final_path = $main_path . DIRECTORY_SEPARATOR . $filename;
                if (!move_uploaded_file($_FILES['challan_for_manufacturer_renewal_upload_challan']['tmp_name'], $final_path)) {
            ...
            if ($payment_type == VALUE_ONE || $payment_type == VALUE_TWO) {
                $error_message = $this->utility_lib->update_fees_bifurcation_details(VALUE_SIXTEEN, $manufacturer_renewal_id, $user_id, $manufacturer_data);
                if ($error_message != '') {
                ...
                $this->utility_model->update_data('module_type', VALUE_SIXTEEN, 'fees_bifurcation', $update_data, 'module_id', $manufacturer_renewal_id);
            }
            $this->utility_model->update_data('manufacturer_renewal_id', $manufacturer_renewal_id, 'wm_manufacturer_renewal', $manufacturer_data);
            $this->db->trans_complete();
            if ($this->db->trans_status() === FALSE) {
                echo json_encode(get_error_array(DATABASE_ERROR_MESSAGE));
                return;
    ...
    function approve_application() {
        try {
            ...
            $session_user_id = get_from_session('temp_id_for_eodbsws_admin');
            $manufacturer_renewal_id = get_from_post('manufacturer_renewal_id_for_manufacturer_renewal_approve');
            if (!is_post() || $session_user_id == NULL || !$session_user_id || !$manufacturer_renewal_id || $manufacturer_renewal_id == NULL) {
                echo json_encode(get_error_array(INVALID_ACCESS_MESSAGE));
                return false;
            }
            $update_data = array();
            $update_data['admin_registration_number'] = get_from_post('registration_number_for_manufacturer_renewal_approve');
            $update_data['valid_upto'] = get_from_post('valid_upto_for_manufacturer_renewal_approve');
            $update_data['remarks'] = get_from_post('remarks_for_manufacturer_renewal_approve');
            ...
            $this->db->trans_start();
            $ex_data = $this->utility_model->get_by_id('manufacturer_renewal_id', $manufacturer_renewal_id, 'wm_manufacturer_renewal');
            if (empty($ex_data)) {
                echo json_encode(get_error_array(INVALID_ACCESS_MESSAGE));
                return false;
            }
            $update_data['processing_days'] = $this->utility_lib->calculate_processing_days(VALUE_SIXTEEN, $ex_data['submitted_datetime']);
            $update_data['status'] = VALUE_FIVE;
            $update_data['status_datetime'] = date('Y-m-d H:i:s');
            $update_data['updated_by'] = $session_user_id;
            $update_data['updated_time'] = date('Y-m-d H:i:s');
            $this->utility_model->update_data('manufacturer_renewal_id', $manufacturer_renewal_id, 'wm_manufacturer_renewal', $update_data);

            $this->utility_lib->send_sms_and_email_for_app_approve($ex_data['user_id'], VALUE_SEVEN, VALUE_SIXTEEN, $manufacturer_renewal_id);
    ...
    function reject_renewal_application() {
        try {
            ...
            $session_user_id = get_from_session('temp_id_for_eodbsws_admin');
            $manufacturer_renewal_id = get_from_post('manufacturer_renewal_id_for_manufacturer_renewal_reject');
            if (!is_post() || $session_user_id == NULL || !$session_user_id || !$manufacturer_renewal_id || $manufacturer_renewal_id == NULL) {
                echo json_encode(get_error_array(INVALID_ACCESS_MESSAGE));
                return false;
            }
            $update_data = array();
            $update_data['remarks'] = get_from_post('remarks_for_manufacturer_renewal_reject');
            if (!$update_data['remarks']) {
                echo json_encode(get_error_array(ESTABLISHMENT_REMARK_MESSAGE));
                return false;
            }
            ...
            $this->utility_lib->send_sms_and_email_for_app_reject($ex_data['user_id'], VALUE_EIGHT, VALUE_SIXTEEN, $manufacturer_renewal_id);
    ...
    function generate_certificate() {
        try {
            ...
            $mpdf = new \Mpdf\Mpdf(['mode' => 'utf-8', 'format' => 'A4']);
            $mpdf->WriteHTML($this->load->view('wmmanufacturer_renewal/certificate', $data, TRUE));
            $mpdf->Output('Manufacturer_Renewal_certificate_' . time() . '.pdf', 'I');
        } catch (\Exception $e) {
            print_r($e->getMessage());
            return false;
        }
    ...
    function generate_excel() {
        try {
            ...
            $session_district = get_from_session('temp_district_for_eodbsws_admin');
            $this->db->trans_start();
            $excel_data = $this->manufacturer_renewal_model->get_records_for_excel($session_district);
            $this->db->trans_complete();
            if ($this->db->trans_status() === false) {
                print_r(INVALID_ACCESS_MESSAGE);
                return;
            ...
            header('Content-Disposition: attachment; filename=Manufacturer_Renewal_Report_' . date('Y-m-d H:i:s') . '.csv');
            ...
            if (!empty($excel_data)) {
                ...
                foreach ($excel_data as $list) {
                    $prefix = isset($prefix_module_array[VALUE_SIXTEEN]) ? $prefix_module_array[VALUE_SIXTEEN] : '';
                    $list['manufacturer_renewal_id'] = generate_registration_number($prefix, $list['manufacturer_renewal_id']);
