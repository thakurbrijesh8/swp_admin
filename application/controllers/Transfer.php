...
class Transfer extends CI_Controller {
    ...
    function get_transfer_data() {
        try {
            ...
            $this->db->trans_start();
            $success_array['transfer_data'] = $this->transfer_model->get_all_transfer_list($start, $length, $search_district, $search_entity_establishment_type, $search_logged_user_detail, $search_applicant_detail, $search_survey_number, $search_ltn, $search_app_timing, $search_status, $search_query_status);
            $success_array['recordsTotal'] = $this->transfer_model->get_total_count_of_records($search_district);
            if (($search_district != '' && (is_admin() || is_view_all_district_user())) || $search_entity_establishment_type || $search_logged_user_detail != '' || $search_applicant_detail != '' || $search_survey_number != '' || $search_ltn != '' || $search_app_timing != '' || $search_status != '' || $search_query_status != '') {
                $success_array['recordsFiltered'] = $this->transfer_model->get_filter_count_of_records($search_district, $search_entity_establishment_type, $search_logged_user_detail, $search_applicant_detail, $search_survey_number, $search_ltn, $search_app_timing, $search_status, $search_query_status);
            } else {
    ...
    function generate_excel() {
        try {
            ...
            $session_district = get_from_session('temp_district_for_eodbsws_admin');
            $this->db->trans_start();
            $excel_data = $this->transfer_model->get_records_for_excel($session_district);
            $this->db->trans_complete();
            if ($this->db->trans_status() === false) {
                print_r(INVALID_ACCESS_MESSAGE);
                return;
            ...
            header('Content-Disposition: attachment; filename=Buyer_of_Plot_Report_' . date('Y-m-d H:i:s') . '.csv');
            ...
            if (!empty($excel_data)) {
                ...
                foreach ($excel_data as $list) {
                    $prefix = isset($prefix_module_array[VALUE_TWELVE]) ? $prefix_module_array[VALUE_TWELVE] : '';
                    $list['transfer_id'] = generate_registration_number($prefix, $list['transfer_id']);
