...
class Wmregistration extends CI_Controller {
    ...
    function __construct() {
        parent::__construct();
        check_authenticated();
        $this->load->model('wmregistration_model');
    }
    ...
    function get_wmregistration_data() {
        try {
            ...
            $this->db->trans_start();
            $success_array['wmregistration_data'] = $this->wmregistration_model->get_all_wmregistration_list($start, $length, $search_district, $search_entity_establishment_type, $search_logged_user_detail, $search_applicant_detail, $search_app_timing, $search_status, $search_query_status);
            $success_array['recordsTotal'] = $this->wmregistration_model->get_total_count_of_records($search_district);
            if (($search_district != '' && (is_admin() || is_view_all_district_user())) || $search_entity_establishment_type || $search_logged_user_detail != '' || $search_applicant_detail != '' || $search_app_timing != '' || $search_status != '' || $search_query_status != '') {
                $success_array['recordsFiltered'] = $this->wmregistration_model->get_filter_count_of_records($search_district, $search_entity_establishment_type, $search_logged_user_detail, $search_applicant_detail, $search_app_timing, $search_status, $search_query_status);
            } else {
                $success_array['recordsFiltered'] = $success_array['recordsTotal'];
            }
            $success_array['query_movements'] = $this->utility_lib->get_query_movement_string($success_array['wmregistration_data'], VALUE_ONE);
    ...
    function get_wmregistration_data_by_id() {
        try {
            ...
            $this->db->trans_start();
            $wmregistration_data = $this->wmregistration_model->get_wmregistration_by_id($wmregistration_id);
    ...
    function get_wmregistration_data_by_wmregistration_id() {
        try {
            ...
            $this->db->trans_start();
            $wmregistration_data = $this->utility_model->get_by_id_with_applicant_name('wmregistration_id', $wmregistration_id, 'wm_registration');
            if (empty($wmregistration_data)) {
                echo json_encode(get_error_array(INVALID_ACCESS_MESSAGE));
                return false;
            }
            if ($is_fb_details == VALUE_ONE || $is_fb_details == VALUE_TWO) {
                $fb_data = $this->utility_model->get_result_data_by_id('module_type', VALUE_ONE, 'fees_bifurcation', 'module_id', $wmregistration_id);
                if ($is_fb_details == VALUE_TWO) {
                    $this->load->model('payment_model');
                    $ph_data = $this->payment_model->get_payment_history(VALUE_ONE, $wmregistration_id);
                }
                if ($wmregistration_data['status'] != VALUE_FOUR && $wmregistration_data['status'] != VALUE_FIVE &&
    ...
    function remove_challan() {
        try {
            ...
            $file_path = 'documents' . DIRECTORY_SEPARATOR . 'wmregistration' . DIRECTORY_SEPARATOR . $ex_est_data['challan'];
            if (file_exists($file_path)) {
                unlink($file_path);
            }
            $this->utility_model->update_data('wmregistration_id', $wmregistration_id, 'wm_registration', array('status' => VALUE_TWO, 'challan' => '', 'updated_by' => $session_user_id, 'updated_time' => date('Y-m-d H:i:s')));
            $success_array = get_success_array();
            $success_array['message'] = DOCUMENT_REMOVED_MESSAGE;
            echo json_encode($success_array);
        } catch (\Exception $e) {
    ...
    function upload_challan() {
        try {
            ...
            $payment_type = get_from_post('payment_type_for_wmregistration_upload_challan');
            ...
            $wmregistration_data = array();
            if ($_FILES['challan_for_wmregistration_upload_challan']['name'] != '') {
                $main_path = 'documents/wmregistration';
                // if (!is_dir($main_path)) {
                //     mkdir($main_path);
                ...
                $temp_filename = str_replace('_', '', $_FILES['challan_for_wmregistration_upload_challan']['name']);
                $filename = 'challan_dd_po_' . (rand(100000000, 999999999)) . time() . '.' . pathinfo($temp_filename, PATHINFO_EXTENSION);
                //Change file name
                $final_path = $main_path . DIRECTORY_SEPARATOR . $filename;
                if (!move_uploaded_file($_FILES['challan_for_wmregistration_upload_challan']['tmp_name'], $final_path)) {
                    echo json_encode(get_error_array(DOCUMENT_NOT_UPLOAD_MESSAGE));
                    return;
                }
                $wmregistration_data['challan'] = $filename;
                $wmregistration_data['challan_updated_date'] = date('Y-m-d H:i:s');
            }
            $wmregistration_data['status'] = VALUE_THREE;
            if ($payment_type == VALUE_THREE) {
                $wmregistration_data['status'] = VALUE_NINE;
            }
            $wmregistration_data['payment_type'] = $payment_type;
            $wmregistration_data['updated_by'] = $user_id;
            $wmregistration_data['updated_time'] = date('Y-m-d H:i:s');
            $wmregistration_data['total_fees'] = VALUE_ZERO;
            $this->db->trans_start();

            if ($payment_type == VALUE_ONE || $payment_type == VALUE_TWO) {
                $error_message = $this->utility_lib->update_fees_bifurcation_details(VALUE_ONE, $wmregistration_id, $user_id, $wmregistration_data);
                if ($error_message != '') {
                ...
                $this->utility_model->update_data('module_type', VALUE_ONE, 'fees_bifurcation', $update_data, 'module_id', $clacwmregistration_idt_id);
            }
            ...
            $success_array['total_fees'] = $wmregistration_data['total_fees'];
            echo json_encode($success_array);
        } catch (\Exception $e) {
            echo json_encode(get_error_array($e->getMessage()));
            return false;
    ...
    function approve_application() {
        try {
            ...
            $session_user_id = get_from_session('temp_id_for_eodbsws_admin');
            $wmregistration_id = get_from_post('wmregistration_id_for_wmregistration_approve');
            if (!is_post() || $session_user_id == NULL || !$session_user_id || !$wmregistration_id || $wmregistration_id == NULL) {
                echo json_encode(get_error_array(INVALID_ACCESS_MESSAGE));
                return false;
            }
            $update_data = array();
            $update_data['admin_registration_number'] = get_from_post('registration_number_for_wmregistration_approve');
            $update_data['valid_upto'] = get_from_post('valid_upto_for_wmregistration_approve');
            $update_data['remarks'] = get_from_post('remarks_for_wmregistration_approve');
            ...
            $this->db->trans_start();
            $ex_data = $this->utility_model->get_by_id('wmregistration_id', $wmregistration_id, 'wm_registration');
            if (empty($ex_data)) {
                echo json_encode(get_error_array(INVALID_ACCESS_MESSAGE));
                return false;
            }
            ...
            $this->utility_model->update_data('wmregistration_id', $wmregistration_id, 'wm_registration', $update_data);

            $this->utility_lib->send_sms_and_email_for_app_approve($ex_data['user_id'], VALUE_SEVEN, VALUE_ONE, $wmregistration_id);
    ...
    function reject_application() {
        try {
            ...
            $session_user_id = get_from_session('temp_id_for_eodbsws_admin');
            $wmregistration_id = get_from_post('wmregistration_id_for_wmregistration_reject');
            if (!is_post() || $session_user_id == NULL || !$session_user_id || !$wmregistration_id || $wmregistration_id == NULL) {
                echo json_encode(get_error_array(INVALID_ACCESS_MESSAGE));
                return false;
            }
            $update_data = array();
            $update_data['remarks'] = get_from_post('remarks_for_wmregistration_reject');
            if (!$update_data['remarks']) {
                echo json_encode(get_error_array(ESTABLISHMENT_REMARK_MESSAGE));
                return false;
            }
            ...
            $this->utility_lib->send_sms_and_email_for_app_reject($ex_data['user_id'], VALUE_EIGHT, VALUE_ONE, $wmregistration_id);
    ...
    function generate_certificate() {
        try {
            ...
            $mpdf->WriteHTML($this->load->view('wmregistration/certificate', $data, TRUE));
            $mpdf->Output('Wmregistration_certificate_' . time() . '.pdf', 'I');
        } catch (\Exception $e) {
            print_r($e->getMessage());
            return false;
        }
    ...
    function generate_excel() {
        try {
            ...
            $session_district = get_from_session('temp_district_for_eodbsws_admin');
            $this->db->trans_start();
            $excel_data = $this->wmregistration_model->get_records_for_excel($session_district);
            $this->db->trans_complete();
            if ($this->db->trans_status() === false) {
                print_r(INVALID_ACCESS_MESSAGE);
                return;
            ...
            header('Content-Disposition: attachment; filename=Wmregistration_Report_' . date('Y-m-d H:i:s') . '.csv');
            ...
            if (!empty($excel_data)) {
                ...
                foreach ($excel_data as $list) {
                    $prefix = isset($prefix_module_array[VALUE_ONE]) ? $prefix_module_array[VALUE_ONE] : '';
                    $list['wmregistration_id'] = generate_registration_number($prefix, $list['wmregistration_id']);
