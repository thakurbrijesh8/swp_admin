...
class Repairer_renewal_model extends CI_Model {

    function get_all_repairer_renewal_list($start, $length, $search_district = '', $search_entity_establishment_type = '', $search_logged_user_detail = '', $search_applicant_detail = '', $search_app_timing = '', $search_status = '', $search_query_status = '') {
        ...
        if ($search_app_timing != '') {
            ...
            $qm_data = isset($query_module_array[VALUE_FOURTEEN]) ? $query_module_array[VALUE_FOURTEEN] : array();
        ...
        $this->db->from('wm_repairer_renewal AS r');
        $this->db->join('users as u', 'u.user_id = r.user_id');
        $this->db->join('sa_users as sau', 'sau.sa_user_id = r.updated_by', 'left');
        $this->db->order_by('r.repairer_renewal_id', 'DESC');
        $resc = $this->db->get();
        //print_r($this->db->last_query());   
        return $resc->result_array();
    }

    function get_repairer_renewal_by_id($repairer_renewal_id) {
        $this->db->where('is_delete !=', IS_DELETE);
        $this->db->where('repairer_renewal_id', $repairer_renewal_id);
        $this->db->from('wm_repairer_renewal');
        $resc = $this->db->get();
        return $resc->row_array();
    }

    function get_total_count_of_records($search_district) {
        $this->db->select('COUNT(repairer_renewal_id) AS total_records');
    ...
    function get_filter_count_of_records($search_district = '', $search_entity_establishment_type = '', $search_logged_user_detail = '', $search_applicant_detail = '', $search_app_timing = '', $search_status = '', $search_query_status = '') {
        $this->db->select('COUNT(r.repairer_renewal_id) AS total_records');
        if ($search_district != '') {
            $this->db->where('r.district', $search_district);
        }
        if ($search_entity_establishment_type != '') {
    ...
    function get_records_for_excel($search_district) {
        $this->db->select('f.repairer_renewal_id, f.district, f.entity_establishment_type, u.applicant_name, u.mobile_number, f.name_of_repairer, '
