...
class Periodicalreturn_model extends CI_Model {

    function get_all_periodicalreturn_list($start, $length, $search_district = '', $search_entity_establishment_type = '', $search_logged_user_detail = '', $search_applicant_detail = '', $search_app_timing = '', $search_status = '', $search_query_status = '') {
        ...
        if ($search_applicant_detail != '') {
            $where = "(r.name_of_applicant  LIKE '%$search_applicant_detail%' OR "
                    . "r.applicant_address  LIKE '%$search_applicant_detail%')";
            $this->db->where($where);
        }
        ...
        $this->db->from('periodicalreturn AS r');
        $this->db->join('users as u', 'u.user_id = r.user_id');
        $this->db->join('sa_users as sau', 'sau.sa_user_id = r.updated_by', 'left');
        $this->db->order_by('r.periodicalreturn_id', 'DESC');
        $resc = $this->db->get();
        //print_r($this->db->last_query());   
        return $resc->result_array();
    }

    function get_periodicalreturn_by_id($periodicalreturn_id) {
        $this->db->where('is_delete !=', IS_DELETE);
        $this->db->where('periodicalreturn_id', $periodicalreturn_id);
        $this->db->from('periodicalreturn');
        $resc = $this->db->get();
        return $resc->row_array();
    }

    function get_total_count_of_records($search_district) {
        $this->db->select('COUNT(periodicalreturn_id) AS total_records');
    ...
    function get_filter_count_of_records($search_district = '', $search_entity_establishment_type = '', $search_logged_user_detail = '', $search_applicant_detail = '', $search_app_timing = '', $search_status = '', $search_query_status = '') {
        $this->db->select('COUNT(r.periodicalreturn_id) AS total_records');
        if ($search_district != '') {
            $this->db->where('r.district', $search_district);
        }
        if ($search_entity_establishment_type != '') {
    ...
    function get_records_for_excel($search_district) {
        $this->db->select('f.periodicalreturn_id, f.district, f.entity_establishment_type, u.applicant_name, u.mobile_number, f.name_of_applicant, '
                . 'f.applicant_address, f.submitted_datetime, f.status, f.query_status, sau.name, f.status_datetime, f.remarks');
