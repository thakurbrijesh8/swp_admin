...
class Filmshooting_model extends CI_Model {

    function get_all_filmshooting_list($start, $length, $search_district = '', $search_entity_establishment_type = '', $search_logged_user_detail = '', $search_applicant_detail = '', $search_app_timing = '', $search_status = '', $search_query_status = '') {
        ...
        if ($search_applicant_detail != '') {
            $where = "(r.production_manager  LIKE '%$search_applicant_detail%' OR "
                    . "r.production_house  LIKE '%$search_applicant_detail%')";
            $this->db->where($where);
        }
        if ($search_app_timing != '') {
            $query_module_array = $this->config->item('query_module_array');
            $qm_data = isset($query_module_array[VALUE_TWENTYTWO]) ? $query_module_array[VALUE_TWENTYTWO] : array();
        ...
        $this->db->from('filmshooting AS r');
        $this->db->join('users as u', 'u.user_id = r.user_id');
        $this->db->join('sa_users as sau', 'sau.sa_user_id = r.updated_by', 'left');
        $this->db->order_by('r.filmshooting_id', 'DESC');
        $resc = $this->db->get();
        //print_r($this->db->last_query());   
        return $resc->result_array();
    }

    function get_filmshooting_by_id($filmshooting_id) {
        $this->db->where('is_delete !=', IS_DELETE);
        $this->db->where('filmshooting_id', $filmshooting_id);
        $this->db->from('filmshooting');
        $resc = $this->db->get();
        return $resc->row_array();
    }

    function get_total_count_of_records($search_district) {
        $this->db->select('COUNT(filmshooting_id) AS total_records');
    ...
    function get_filter_count_of_records($search_district = '', $search_entity_establishment_type = '', $search_logged_user_detail = '', $search_applicant_detail = '', $search_app_timing = '', $search_status = '', $search_query_status = '') {
        $this->db->select('COUNT(r.filmshooting_id) AS total_records');
        if ($search_district != '') {
            $this->db->where('r.district', $search_district);
        }
        if ($search_entity_establishment_type != '') {
    ...
    function get_records_for_excel($search_district) {
        $this->db->select('r.filmshooting_id, r.district, r.entity_establishment_type, u.applicant_name, u.mobile_number, r.production_house, r.address, '
                . 'r.contact_no, r.submitted_datetime, r.status, r.query_status, sau.name, r.status_datetime, r.remarks');
