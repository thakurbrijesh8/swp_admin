...
class Migrantworkers_renewal_model extends CI_Model {

    function get_all_migrantworkers_renewal_list($start, $length, $search_district = '', $search_entity_establishment_type = '', $search_logged_user_detail = '', $search_applicant_detail = '', $search_app_timing = '', $search_status = '', $search_query_status = '') {
        $this->db->select("r.*,u.applicant_name,u.mobile_number AS applicant_mobile,date_format(r.created_time, '%d-%m-%Y %H:%i:%s') AS display_datetime,sau.name AS logged_user_name,sau.name AS logged_user_name");
        if ($search_district != '') {
        ...
        if ($search_applicant_detail != '') {
            $where = "(r.name_of_establishment  LIKE '%$search_applicant_detail%' OR "
                    . "r.nature_of_work_of_establishment  LIKE '%$search_applicant_detail%')";
            $this->db->where($where);
        }
        if ($search_app_timing != '') {
            $query_module_array = $this->config->item('query_module_array');
            $qm_data = isset($query_module_array[VALUE_FOURTYFIVE]) ? $query_module_array[VALUE_FOURTYFIVE] : array();
        ...
        $this->db->from('migrantworkers_renewal AS r');
        $this->db->join('users as u', 'u.user_id = r.user_id');
        $this->db->join('sa_users as sau', 'sau.sa_user_id = r.updated_by', 'left');
        $this->db->order_by('r.migrantworkers_renewal_id', 'DESC');
        $resc = $this->db->get();
        return $resc->result_array();
    }

    function get_migrantworkers_renewal_by_id($migrantworkers_renewal_id) {
        $this->db->where('is_delete !=', IS_DELETE);
        $this->db->where('migrantworkers_renewal_id', $migrantworkers_renewal_id);
        $this->db->from('migrantworkers_renewal');
        $resc = $this->db->get();
        return $resc->row_array();
    }

    function get_total_count_of_records($search_district) {
        $this->db->select('COUNT(migrantworkers_renewal_id) AS total_records');
    ...
    function get_filter_count_of_records($search_district = '', $search_entity_establishment_type = '', $search_logged_user_detail = '', $search_applicant_detail = '', $search_app_timing = '', $search_status = '', $search_query_status = '') {
        $this->db->select('COUNT(r.migrantworkers_renewal_id) AS total_records');
        if ($search_district != '') {
            $this->db->where('r.district', $search_district);
        }
        if ($search_entity_establishment_type != '') {
    ...
    function get_records_for_excel($search_district) {
        $this->db->select('r.migrantworkers_renewal_id, r.district, r.entity_establishment_type, u.applicant_name, u.mobile_number, r.name_of_establishment, '
                . 'r.nature_of_work_of_establishment, r.submitted_datetime, r.status, r.query_status, sau.name, r.status_datetime, r.remarks');
...
/*
 * EOF: ./application/models/Migrantworkers_renewal_model.php
 */