...
class Travelagent_renewal_model extends CI_Model {

    function get_all_travelagent_renewal_list($start, $length, $search_district = '', $search_entity_establishment_type = '', $search_logged_user_detail = '', $search_applicant_detail = '', $search_app_timing = '', $search_status = '', $search_query_status = '') {
        ...
        if ($search_district != '') {
            $this->db->where('r.area_of_agency', $search_district);
        }
        ...
        if ($search_applicant_detail != '') {
            $where = "(r.name_of_travel_agency  LIKE '%$search_applicant_detail%' OR "
                    . "r.address_of_agency  LIKE '%$search_applicant_detail%')";
            $this->db->where($where);
        }
        if ($search_app_timing != '') {
            $query_module_array = $this->config->item('query_module_array');
            $qm_data = isset($query_module_array[VALUE_TWENTYTHREE]) ? $query_module_array[VALUE_TWENTYTHREE] : array();
        ...
        $this->db->from('travelagent_renewal AS r');
        $this->db->join('users as u', 'u.user_id = r.user_id');
        $this->db->join('sa_users as sau', 'sau.sa_user_id = r.updated_by', 'left');
        $this->db->order_by('r.travelagent_renewal_id', 'DESC');
        $resc = $this->db->get();
        return $resc->result_array();
    }

    function get_travelagent_renewal_by_id($travelagent_renewal_id) {
        $this->db->where('is_delete !=', IS_DELETE);
        $this->db->where('travelagent_renewal_id', $travelagent_renewal_id);
        $this->db->from('travelagent_renewal');
        $resc = $this->db->get();
        return $resc->row_array();
    }

    function get_total_count_of_records($search_district) {
        $this->db->select('COUNT(travelagent_renewal_id) AS total_records');
        if (!is_admin() && !is_view_all_district_user()) {
            if ($search_district != '') {
                $this->db->where('area_of_agency', $search_district);
            }
        }
        $this->db->where('is_delete !=' . IS_DELETE);
    ...
    function get_filter_count_of_records($search_district = '', $search_entity_establishment_type = '', $search_logged_user_detail = '', $search_applicant_detail = '', $search_app_timing = '', $search_status = '', $search_query_status = '') {
        ...
        $this->db->select('COUNT(r.travelagent_renewal_id) AS total_records');
        if ($search_district != '') {
            $this->db->where('r.area_of_agency', $search_district);
        }
        if ($search_entity_establishment_type != '') {
    ...
    function get_records_for_excel($search_district) {
        $this->db->select('t.travelagent_renewal_id, t.area_of_agency, t.entity_establishment_type, u.applicant_name, u.mobile_number, t.name_of_travel_agency, '
                . 't.address_of_agency, t.submitted_datetime, t.status, t.query_status, sau.name, t.status_datetime, t.remarks');
...
/*
 * EOF: ./application/models/Travelagent_renewal_model.php
 */