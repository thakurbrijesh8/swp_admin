...
class Landallotment_model extends CI_Model {

    function get_all_landallotment_list($start, $length, $search_district = '', $search_entity_establishment_type = '', $search_logged_user_detail = '', $search_applicant_detail = '', $search_app_timing = '', $search_status = '', $search_query_status = '') {
        ...
        if ($search_app_timing != '') {
            ...
            $qm_data = isset($query_module_array[VALUE_TWENTYFIVE]) ? $query_module_array[VALUE_TWENTYFIVE] : array();
        ...
        $this->db->from('land_allotment AS r');
        $this->db->join('users as u', 'u.user_id = r.user_id');
        $this->db->join('sa_users as sau', 'sau.sa_user_id = r.updated_by', 'left');
        $this->db->order_by('r.landallotment_id', 'DESC');
        $resc = $this->db->get();
        //print_r($this->db->last_query());   
        return $resc->result_array();
    }

    function get_landallotment_by_id($landallotment_id) {
        $this->db->where('is_delete !=', IS_DELETE);
        $this->db->where('landallotment_id', $landallotment_id);
        $this->db->from('land_allotment');
        $resc = $this->db->get();
        return $resc->row_array();
    }

    function get_total_count_of_records($search_district) {
        $this->db->select('COUNT(landallotment_id) AS total_records');
    ...
    function get_filter_count_of_records($search_district = '', $search_entity_establishment_type = '', $search_logged_user_detail = '', $search_applicant_detail = '', $search_app_timing = '', $search_status = '', $search_query_status = '') {
        $this->db->select('COUNT(r.landallotment_id) AS total_records');
        if ($search_district != '') {
            $this->db->where('r.district', $search_district);
        }
        if ($search_entity_establishment_type != '') {
    ...
    function get_records_for_excel($search_district) {
        $this->db->select('r.landallotment_id, r.district, r.entity_establishment_type, u.applicant_name, u.mobile_number, r.name_of_applicant, '
                . 'r.applicant_address, r.submitted_datetime, r.status, r.query_status, sau.name, r.status_datetime, r.remarks');
