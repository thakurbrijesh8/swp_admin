...
class Travelagent_model extends CI_Model {
    ...
    function get_all_travelagent_list($start, $length, $search_district = '', $search_entity_establishment_type = '', $search_logged_user_detail = '', $search_applicant_detail = '', $search_app_timing = '', $search_status = '', $search_query_status = '') {
        ...
        if ($search_district != '') {
            $this->db->where('t.area_of_agency', $search_district);
        }
        ...
        if ($search_applicant_detail != '') {
            $where = "(t.name_of_person  LIKE '%$search_applicant_detail%' OR "
                    . "t.name_of_travel_agency  LIKE '%$search_applicant_detail%')";
            $this->db->where($where);
        }
        if ($search_app_timing != '') {
            $query_module_array = $this->config->item('query_module_array');
            $qm_data = isset($query_module_array[VALUE_NINETEEN]) ? $query_module_array[VALUE_NINETEEN] : array();
        ...
        $this->db->from('travelagent AS t');
        $this->db->join('users as u', 'u.user_id = t.user_id');
        $this->db->join('sa_users as sau', 'sau.sa_user_id = t.updated_by', 'left');
        $this->db->order_by('t.travelagent_id', 'DESC');
        $resc = $this->db->get();
        return $resc->result_array();
    }

    function get_travelagent_by_id($travelagent_id) {
        $this->db->where('is_delete !=', IS_DELETE);
        $this->db->where('travelagent_id', $travelagent_id);
        $this->db->from('travelagent');
        $resc = $this->db->get();
        return $resc->row_array();
    }

    function get_total_count_of_records($search_district) {
        $this->db->select('COUNT(travelagent_id) AS total_records');
    ...
    function get_filter_count_of_records($search_district = '', $search_entity_establishment_type = '', $search_logged_user_detail = '', $search_applicant_detail = '', $search_app_timing = '', $search_status = '', $search_query_status = '') {
        ...
        $this->db->select('COUNT(t.travelagent_id) AS total_records');
        if ($search_district != '') {
            $this->db->where('t.area_of_agency', $search_district);
        }
        if ($search_entity_establishment_type != '') {
    ...
    function get_records_for_excel($search_district) {
        $this->db->select('t.travelagent_id, t.area_of_agency, t.entity_establishment_type, u.applicant_name, u.mobile_number, t.name_of_person, '
                . 't.name_of_travel_agency, t.submitted_datetime, t.status, t.query_status, sau.name, t.status_datetime, t.remarks');
...
/*
 * EOF: ./application/models/Travelagent_model.php
 */