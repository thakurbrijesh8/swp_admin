...
class Aplicence_model extends CI_Model {

    function get_all_aplicence_list($start, $length, $search_district = '', $search_entity_establishment_type = '', $search_logged_user_detail = '', $search_applicant_detail = '', $search_app_timing = '', $search_status = '', $search_query_status = '') {
        ...
        if ($search_applicant_detail != '') {
            $where = "(r.contractor_name  LIKE '%$search_applicant_detail%' OR "
                    . "r.nature_of_process_for_establi  LIKE '%$search_applicant_detail%')";
            $this->db->where($where);
        }
        if ($search_app_timing != '') {
            $query_module_array = $this->config->item('query_module_array');
            $qm_data = isset($query_module_array[VALUE_FOURTYTHREE]) ? $query_module_array[VALUE_FOURTYTHREE] : array();
        ...
        $this->db->from('appli_licence AS r');
        $this->db->join('users as u', 'u.user_id = r.user_id');
        $this->db->join('sa_users as sau', 'sau.sa_user_id = r.updated_by', 'left');
        $this->db->order_by('r.aplicence_id', 'DESC');
        $resc = $this->db->get();
        //print_r($this->db->last_query());   
        return $resc->result_array();
    }

    function get_aplicence_by_id($aplicence_id) {
        $this->db->where('is_delete !=', IS_DELETE);
        $this->db->where('aplicence_id', $aplicence_id);
        $this->db->from('appli_licence');
        $resc = $this->db->get();
        return $resc->row_array();
    }

    function get_total_count_of_records($search_district) {
        $this->db->select('COUNT(aplicence_id) AS total_records');
    ...
    function get_filter_count_of_records($search_district = '', $search_entity_establishment_type = '', $search_logged_user_detail = '', $search_applicant_detail = '', $search_app_timing = '', $search_status = '', $search_query_status = '') {
        $this->db->select('COUNT(r.aplicence_id) AS total_records');
        if ($search_district != '') {
            $this->db->where('r.district', $search_district);
        }
        if ($search_entity_establishment_type != '') {
    ...
    function get_records_for_excel($search_district) {
        $this->db->select('a.aplicence_id, a.district, a.entity_establishment_type, u.applicant_name, u.mobile_number, a.contractor_name, '
                . 'a.nature_of_process_for_establi, a.submitted_datetime, a.status, a.query_status, sau.name, a.status_datetime, a.remarks');
        if ($search_district != VALUE_ZERO) {
            $this->db->where('a.district', $search_district);
        }
        $this->db->where('a.is_delete !=' . IS_DELETE);
        $this->db->where('a.status != ' . VALUE_ZERO);
        $this->db->where('a.status != ' . VALUE_ONE);
        $this->db->from('appli_licence AS a');
        $this->db->join('users as u', 'u.user_id = a.user_id');
        $this->db->join('sa_users as sau', 'sau.sa_user_id = a.updated_by', 'left');
        $this->db->order_by('a.aplicence_id', 'DESC');
        $resc = $this->db->get();
        return $resc->result_array();
    }
