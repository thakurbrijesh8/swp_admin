...
class Property_model extends CI_Model {
    ...
    function get_all_Property_list($start, $length, $search_district = '', $search_entity_establishment_type = '', $search_logged_user_detail = '', $search_applicant_detail = '', $search_app_timing = '', $search_status = '', $search_query_status = '') {
        ...
        if ($search_applicant_detail != '') {
            $where = "(r.party_name  LIKE '%$search_applicant_detail%' OR "
                    . "r.party_address  LIKE '%$search_applicant_detail%')";
            $this->db->where($where);
        }
        if ($search_app_timing != '') {
            $query_module_array = $this->config->item('query_module_array');
            $qm_data = isset($query_module_array[VALUE_TWENTYONE]) ? $query_module_array[VALUE_TWENTYONE] : array();
    ...
    function get_Property_by_id($property_id) {
        ...
        $this->db->from('property_registration');
        $resc = $this->db->get();
        return $resc->row_array();
    }

    function get_total_count_of_records($search_district) {
        $this->db->select('COUNT(property_id) AS total_records');
    ...
    function get_filter_count_of_records($search_district = '', $search_entity_establishment_type = '', $search_logged_user_detail = '', $search_applicant_detail = '', $search_app_timing = '', $search_status = '', $search_query_status = '') {
        $this->db->select('COUNT(r.property_id) AS total_records');
        if ($search_district != '') {
            $this->db->where('r.district', $search_district);
        }
        if ($search_entity_establishment_type != '') {
    ...
    function get_records_for_excel($search_district) {
        $this->db->select('f.property_id, f.district, f.entity_establishment_type, u.applicant_name, u.mobile_number, f.party_name, '
                . 'f.party_address, f.submitted_datetime, f.status, f.query_status, sau.name, f.status_datetime, f.remarks');
