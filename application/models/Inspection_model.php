...
class Inspection_model extends CI_Model {
    ...
    function get_all_inspection_list($start, $length, $search_district = '', $search_entity_establishment_type = '', $search_logged_user_detail = '', $search_applicant_detail = '', $search_app_timing = '', $search_status = '', $search_query_status = '') {
        ...
        if ($search_applicant_detail != '') {
            $where = "(r.name_of_applicant  LIKE '%$search_applicant_detail%' OR "
                    . "r.address  LIKE '%$search_applicant_detail%')";
            $this->db->where($where);
        }
        if ($search_app_timing != '') {
            $query_module_array = $this->config->item('query_module_array');
            $qm_data = isset($query_module_array[VALUE_TWENTYSEVEN]) ? $query_module_array[VALUE_TWENTYSEVEN] : array();
    ...
    function get_total_count_of_records($search_district) {
        $this->db->select('COUNT(inspection_id) AS total_records');
    ...
    function get_filter_count_of_records($search_district = '', $search_entity_establishment_type = '', $search_logged_user_detail = '', $search_applicant_detail = '', $search_app_timing = '', $search_status = '', $search_query_status = '') {
        $this->db->select('COUNT(r.inspection_id) AS total_records');
        if ($search_district != '') {
            $this->db->where('r.district', $search_district);
        }
        if ($search_entity_establishment_type != '') {
    ...
    // $this->db->select('r.inspection_id, r.district, u.applicant_name, u.mobile_number, r.name_of_applicant, '
    //            . 'r.village, r.submitted_datetime, r.status, r.query_status');
    function get_records_for_excel($search_district) {
        $this->db->select('r.inspection_id, r.district, r.entity_establishment_type, u.applicant_name, u.mobile_number, r.name_of_applicant, '
                . 'r.village, r.submitted_datetime, r.status, r.query_status, sau.name, r.status_datetime, r.remarks');
