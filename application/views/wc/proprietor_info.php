<tr id="proprietor_info_{{per_cnt}}" class="proprietor_info" style="background-color: #fff;">
    ...
    <td>
        <input type="text" id="father_name_{{per_cnt}}" name="father_name_{{per_cnt}}" class="form-control"
               maxlength="100" onblur="checkValidation('repairer', 'father_name_{{per_cnt}}', fatherNameValidationMessage);" value="{{father_name}}">
        <span class="error-message error-message-repairer-father_name_{{per_cnt}}"></span>
    </td>
    <td>
    ...
    <td class="text-center">
        <button type="button" class="btn btn-xs btn-danger" style="margin-top: 2px;" onclick="Repairer.listview.removeProprietorInfo({{per_cnt}});">
