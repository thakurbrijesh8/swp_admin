<?php
$barcode_number = generate_barcode_number(VALUE_FOURTEEN, $repairer_renewal_data['repairer_renewal_id']);
...
<div style="font-size: 14px; text-align: left;word-spacing: 10px;">license No : <?php echo $repairer_renewal_data['admin_registration_number']; ?></div>
<div style="font-size: 14px; text-align: right; margin-bottom: 5px;">Year : &emsp;<?php echo date("Y"); ?></div>

<div style="font-size: 14px;text-align: left;margin-top: 5px;word-spacing: 5px;">(1)&emsp;&emsp;The controller of legal metrology hereby grants to<b> <?php echo $repairer_renewal_data['name_of_repairer']; ?>&nbsp;&nbsp;<?php echo $repairer_renewal_data['complete_address']; ?></b> a license to repair the following :-<br/>(Include details of the types of weights,measures,weighing instruments or measuring instruments that are license to be reapaired by the party).</div>
<br />
<div style="font-size: 14px;text-align: left;margin-top: 5px;word-spacing: 5px;">(2)&emsp;&emsp;The license is valid for the party named above in respect of his workshop located at <?php echo $repairer_renewal_data['complete_address']; ?>.</div>
<br />
<div style="font-size: 14px;text-align: left;margin-top: 5px;word-spacing: 5px;">(3)&emsp;&emsp;The license is valid upto <?php echo date('d/m/Y',strtotime($repairer_renewal_data['valid_upto'])); ?> .</div>
<br />
...
<table style="margin-left: 490px;">
    <tr><td class="border-none"><img src="<?php echo base_url(); ?>documents/repairer/<?php echo  $repairer_renewal_data['mw_sign_of_principal_employer']; ?>" height="60px" width="90px"></td></tr>
</table>
