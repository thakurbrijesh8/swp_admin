...
var sublettingViewPaymentTemplate = Handlebars.compile($('#subletting_view_payment_template').html());
...
Subletting.listView = Backbone.View.extend({
    ...
    openUploadChallan: function (sublettingId) {
        ...
        $.ajax({
            ...
            type: 'post',
            data: $.extend({}, {'subletting_id': sublettingId, 'load_fb_details': VALUE_ONE}, getTokenData()),
            error: function (textStatus, errorThrown) {
                generateNewCSRFToken();
                btnObj.html(ogBtnHTML);
                btnObj.attr('onclick', ogBtnOnclick);
...
//                }
                if (sublettingData.payment_type == VALUE_ONE) {
                    sublettingData.utitle = 'Challan Copy';
                } else {
                    sublettingData.utitle = 'Payment Details';
                }
                sublettingData.module_type = VALUE_THIRTEEN;
                $('#popup_container').html(sublettingUploadChallanTemplate(sublettingData));
                loadFB(VALUE_THIRTEEN, parseData.fb_data, sublettingData.payment_type, sublettingData.show_remove_upload_btn, sublettingData.show_dropdown, sublettingData.dropdown_data);

                generateBoxes('radio', paymentTypeArray, 'payment_type', 'subletting_upload_challan', sublettingData.payment_type, true);
                showSubContainerForPaymentDetails('payment_type', 'subletting_upload_challan', 'uc', 'radio', '#fb', VALUE_THIRTEEN);
    ...
    uploadChallan: function () {
        ...
        var paymentType = $('input[name=payment_type_for_subletting_upload_challan]:checked').val();
        if (paymentType != VALUE_ONE && paymentType != VALUE_TWO && paymentType != VALUE_THREE) {
            $('#payment_type_for_subletting_upload_challan_1').focus();
            validationMessageShow('subletting-uc-payment_type_for_subletting_upload_challan', onePaymentOptionValidationMessage);
            return false;
        }
        ...
        if (paymentType == VALUE_ONE || paymentType == VALUE_TWO) {
            var returnData = checkValidationForFB(VALUE_THIRTEEN, 'subletting-uc', true);
            if (!returnData) {
                return false;
            }
        }
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                Swal.close();
                $('#status_' + sublettingId).html(paymentType == VALUE_THREE ? appStatusArray[VALUE_NINE] : appStatusArray[VALUE_THREE]);
                if (paymentType == VALUE_THREE) {
                    $('#confirm_payment_btn_for_app_' + sublettingId).show();
                }
                $('#total_fees_' + sublettingId).html(returnFees(parseData));
                showSuccess(parseData.message);
            }
        });
    },
    ...
    viewPayment: function (sublettingId) {
        if (!sublettingId) {
            showError(invalidAccessValidationMessage);
            return false;
        }
        var that = this;
        var btnObj = $('#download_fees_paid_challan_btn_' + sublettingId);
        var ogBtnHTML = btnObj.html();
        var ogBtnOnclick = btnObj.attr('onclick');
        btnObj.html(iconSpinnerTemplate);
        btnObj.attr('onclick', '');
        $.ajax({
            url: 'subletting/get_subletting_data_by_subletting_id',
            type: 'post',
            data: $.extend({}, {'subletting_id': sublettingId, 'load_fb_details': VALUE_TWO}, getTokenData()),
            error: function (textStatus, errorThrown) {
                generateNewCSRFToken();
                btnObj.html(ogBtnHTML);
                btnObj.attr('onclick', ogBtnOnclick);
            ...
            success: function (response) {
                ...
                var sublettingData = parseData.subletting_data;
                showPopup();
                if (sublettingData.payment_type == VALUE_ONE || sublettingData.payment_type == VALUE_THREE) {
                    sublettingData.user_payment_type_text = paymentTypeArray[sublettingData.payment_type];
                } else {
                    sublettingData.user_payment_type_text = userPaymentTypeArray[sublettingData.user_payment_type] ? userPaymentTypeArray[sublettingData.user_payment_type] : '';
                }
                if (sublettingData.payment_type == VALUE_ONE) {
                    sublettingData.utitle = 'Fees Paid Challan Copy';
                } else if (sublettingData.payment_type == VALUE_TWO && sublettingData.user_payment_type == VALUE_ONE) {
                    sublettingData.utitle = 'Demand Draft (DD) Copy';
                }
                sublettingData.module_type = VALUE_THIRTEEN;
                $('#popup_container').html(sublettingViewPaymentTemplate(sublettingData));
                loadFB(VALUE_THIRTEEN, parseData.fb_data, sublettingData.payment_type);
                loadPH(VALUE_THIRTEEN, sublettingData.subletting_id, parseData.ph_data);
                if (sublettingData.payment_type == VALUE_ONE || (sublettingData.payment_type == VALUE_TWO && sublettingData.user_payment_type == VALUE_ONE)) {
                    if (sublettingData.fees_paid_challan != '') {
                        $('#vp_container_for_subletting').show();
                        $('#fees_paid_challan_name_href_for_subletting').attr('href', SUBLETTING_DOC_PATH + sublettingData.fees_paid_challan);
                        $('#fees_paid_challan_name_for_subletting').html(sublettingData.fees_paid_challan);
                    }
                }
            }
        });
